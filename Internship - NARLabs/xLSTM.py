...
class sLSTM(nn.Module):
    ...
    def __init__(
        ...
        self.W = nn.ModuleList([nn.Linear(input_size, head_num * head_size)] * 4)

        self.R = nn.ModuleList([BlockLinear([(head_size, head_size)] * head_num)] * 4)
    ...
    def init_hidden(self, bs: int) -> Tuple[Tensor, Tensor, Tensor, Tensor]:
        ...
        c, h, m = [
            torch.zeros(bs, self.head_num * self.head_size, device=self.device)
        ] * 3
        return c, n, h, m
    ...
    def forward(
        ...
        z, o = [W(x) + R(h_) for W, R in zip(self.W[:-2], self.R[:-2])]
        ...
        o = torch.sigmoid(o)  # Eq. (14) in ref. paper

        # Update the internal states of the model
        c = f * c_ + i * z  # Eq. (8) in ref. paper
        n = f * n_ + i  # Eq. (9) in ref. paper
        h = o * (c / n)  # Eq. (10) in ref. paper
...
class mLSTM(nn.Module):
    ...
    def forward(
        ...
        f: Tensor = self.W_f(x_conv)  # shape: b h
...
class xLSTM(nn.Module):
    ...
    def __init__(
        ...
        self.layers: List[mLSTM | sLSTM] = nn.ModuleList(
            [
                mLSTM(**mlstm_config) if v else sLSTM(**slstm_config)
